...
%#################################################
allowed(north, pos(R,C)) :-
  R>1,
  R1 is R-1,
  \+occupied(pos(R1,C)).

allowed(south, pos(R,C)) :-
  \+row_num(R),
  R1 is R+1,
  \+occupied(pos(R1,C)).

allowed(west, pos(R,C)) :-
  C>1,
  C1 is C-1,
  \+occupied(pos(R,C1)).

allowed(east, pos(R,C)) :-
  \+col_num(C),
  C1 is C+1,
  \+occupied(pos(R,C1)).
...
%###################################################
move(east, pos(R,C), pos(R, CAdjacent)) :-
  CAdjacent is C+1.
move(west, pos(R,C), pos(R, CAdjacent)) :-
  CAdjacent is C-1.
move(north, pos(R,C), pos(RUp,C)) :-
  RUp is R-1.
move(south, pos(R,C), pos(RDown,C)) :-
  RDown is R+1.
